...
<div class="tw-relative tw-bg-gray-50 tw-pt-16 tw-pb-20 tw-px-4 sm:tw-px-6 lg:tw-pt-24 lg:tw-pb-28 lg:tw-px-8">
  ...
  <div class="tw-relative tw-max-w-7xl tw-mx-auto">
    ...
    <div class="tw-mt-3 tw-max-w-2xl tw-mx-auto tw-flex tw-justify-center">
      <div class="tw-inline-flex tw-items-center tw-mx-1 tw-px-2 tw-py-1 tw-rounded-md tw-text-sm tw-font-medium tw-bg-gray-100 tw-text-gray-800 hover:tw-bg-gray-300">
      <a class="tw-h-auto tw-w-full tw-h-full tw-cursor-pointer" href="<?php echo get_taxonomy_archive_link('blogs'); ?>#articles">Blogs</a>
      </div>
      <?php
        $pageTerm = get_term_by( 'slug', get_query_var( 'term' ), get_query_var( 'taxonomy' ) );  
        foreach(get_terms('blogs') as $term){       
          if( $term->slug == $pageTerm->slug){?>
            <div class="tw-inline-flex tw-items-center tw-mx-1 tw-px-2 tw-py-1 tw-rounded-md tw-text-sm tw-font-medium tw-bg-primary-red-300 tw-text-white">
              <div class="tw-text-lg tw-h-auto tw-w-full tw-h-full tw-select-none"><?php echo $term->name;?>s</div>
            </div>
          <?php } else {
      ?>
        <div class="tw-inline-flex tw-items-center tw-px-2 tw-mx-1 tw-py-1 tw-rounded-md tw-text-sm tw-font-medium tw-bg-gray-100 tw-text-gray-800 hover:tw-bg-gray-300">
          <a class="tw-h-auto tw-w-full tw-h-full tw-cursor-pointer" href="<?php echo get_term_link($term->slug,'blogs'); ?>#articles"><?php echo $term->name; ?>s</a>
        </div>
        <?php }
      } ?>
    </div>
      ...
      while ( have_posts() ) : the_post(); ?>
        <div class="tw-flex tw-flex-col tw-rounded-lg tw-shadow-lg tw-overflow-hidden">
          <div class="tw-flex-shrink-0">
            <a href="<?php echo get_permalink(); ?>" class="tw-h-auto tw-cursor-pointer">
          ...
          <div class="tw-flex-1 tw-bg-white tw-p-6 tw-flex tw-flex-col tw-justify-between" href="#">
            <div class="tw-flex-1">
                <p class="tw-text-sm tw-font-medium tw-text-primary-red-300 tw-my-0">
                  <?php foreach(wp_get_post_terms(get_the_ID(), 'blogs') as $itm){ ?>
                    <a class="tw-cursor-pointer tw-mr-4 tw-h-auto tw-text-left tw-text-sm tw-w-auto tw-inline hover:tw-underline" href="<?php echo get_term_link($itm->slug,'blogs'); ?>"><?php echo $itm->name; ?></a>
                  <?php } ?>
                </p>
